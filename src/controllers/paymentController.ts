...
export class PaymentController {
  ...
  static async createSubscription(req: FastifyRequest, res: FastifyReply) {
    try {
      const { customerId, priceId } = req.body as { customerId: string; priceId: string; userId: string };
      ...
      res.status(400).send({ error: 'Erreur lors de la création de l\'abonnement' });
    }
  }
  ...
  static async createDonation(req: FastifyRequest, res: FastifyReply) {
    try {
      const { amount, paymentMethodId, userId } = req.body as { amount: number; paymentMethodId: string; userId: string };
  ...
  static async updatePaymentMethod(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      res.status(400).send({ error: 'Erreur lors de la mise à jour du moyen de paiement' });
    }
  }
  ...
  static async cancelSubscription(req: FastifyRequest, res: FastifyReply) {
    try {
      ...
      res.status(400).send({ error: 'Erreur lors de la résiliation de l\'abonnement' });
    }
  }
    ...
    try {
      ...
      switch (event.type) {
        ...
        case 'invoice.payment_failed':
          ...
          await PaymentController.handleFailedPayment(event.data.object);
          break;
      }
